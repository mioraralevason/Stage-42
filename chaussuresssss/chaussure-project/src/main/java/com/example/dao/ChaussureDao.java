...
@Override
public boolean save(Chaussure chaussure) {
    String sql = "INSERT INTO chaussures (photo, nom, prix, description, stock, " +
                 "taille_id, couleur_id, marque_id, categorie_id, type_chaussure_id) " +
                 "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    try (Connection conn = DatabaseConnection.getConnection();
         PreparedStatement stmt = conn.prepareStatement(sql)) {
        
        stmt.setString(1, chaussure.getPhoto());
        stmt.setString(2, chaussure.getNom());
        stmt.setDouble(3, chaussure.getPrix());
        stmt.setString(4, chaussure.getDescription());
        stmt.setInt(5, chaussure.getStock());
        stmt.setInt(6, chaussure.getTailleId());
        stmt.setInt(7, chaussure.getCouleurId());
        stmt.setInt(8, chaussure.getMarqueId());
        stmt.setInt(9, chaussure.getCategorieId());
        stmt.setInt(10, chaussure.getTypeId());
